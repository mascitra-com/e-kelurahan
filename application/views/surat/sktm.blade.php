...
<div class="panel panel-theme">
	<div class="panel-heading">
		...
		<div class="btn-group pull-right">
			...
			<a href="{{ site_url('sktm_sekolah/arsip') }}" class="btn btn-default btn-sm"><i class="fa fa-archive"></i></a>
		</div>
		...
		<table class="table table-stripped table-hover table-bordered">
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-center">{{ (is_null($surat->no_surat)) ? '-' : $surat->no_surat }}</td>
					...
					<td class="text-center">{{ is_null($surat->tanggal_ambil) ? '-' : date('d-m-Y', strtotime($surat->tanggal_ambil)) }}</td>
					<td class="text-center">{{ is_null($surat->nama_pengambil) ? '-' : $surat->nama_pengambil }}</td>
					<td class="text-center"><span class="label label-{{ (!is_null($surat->nama_pengambil)) ? 'success' : (( date('d-m-Y') > date('d-m-Y', strtotime($surat->tanggal_verif. ' + 7 days')) ) ? 'danger' : 'warning') }}">
						{{ (!is_null($surat->nama_pengambil)) ? 'diambil' : (( date('d-m-Y') > date('d-m-Y', strtotime($surat->tanggal_verif. ' + 7 days')) ) ? 'kadaluarsa' : 'menunggu' ) }}
					</span></td>
					<td>
						@if(is_null($surat->nama_pengambil) && !( date('d-m-Y') > date('d-m-Y', strtotime($surat->tanggal_verif. ' + 7 days')) ))
						<button class="btn btn-block btn-success btn-xs btn-ambil" title="telah diambil" data-toggle="modal" data-target="#modal-ambil" data-id_surat={{ $surat->id }}><i class="fa fa-check"></i></button>	
						@elseif(is_null($surat->nama_pengambil) && ( date('d-m-Y') > date('d-m-Y', strtotime($surat->tanggal_verif. ' + 7 days')) ))
						<a href="{{ site_url('surat/arsipkan/'.$surat->id) }}" class="btn btn-block btn-default btn-xs" onclick="return confirm('Anda yakin?')" title="arsipkan"><i class="fa fa-archive"></i></a>
						@else
						<a href="{{site_url('sktm_sekolah/detail/'.$surat->id)}}" class="btn btn-default btn-xs" title="selengkapnya"><i class="fa fa-info"></i></a>
						<a href="{{site_url('sktm_sekolah/cetak/'.$surat->id)}}" target="_blank" class="btn btn-default btn-xs" title="cetak"><i class="fa fa-print"></i></a>
						<a href="{{ site_url('sktm_sekolah/arsipkan/'.$surat->id) }}" class="btn btn-default btn-xs" onclick="return confirm('Anda yakin?')" title="arsipkan"><i class="fa fa-archive"></i></a>
						@endif
					</td>
				</tr>
				@endif
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{ site_url('sktm_sekolah/simpan') }}" method="POST">
					{{ $csrf }}
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center">{{ $surat->nik }}</td>
							<td>{{ $surat->penduduk->nama }}</td>
							<td class="text-center">{{ $surat->jurusan }}</td>
							<td>{{ $surat->asal_sekolah }}</td>
							<td class="text-center">{{date('d-m-Y', strtotime($surat->created_at))}}</td>
							<td class="text-center">
								<button class="btn btn-sm btn-success" data-konfirmasi="1" data-id="{{ $surat->id }}"><i class="fa fa-check space-right-5"></i>setuju</button>
								<button class="btn btn-sm btn-danger" data-konfirmasi="2" data-id="{{ $surat->id }}"><i class="fa fa-times space-right-5"></i>tolak</button>
							</td>
						</tr>
						@endif
						@endforeach
...
<div class="modal fade" tabindex="-1" role="dialog" id="modal-ambil">
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{ site_url('sktm_sekolah/ambil') }}" method="POST" class="form">
					{{ $csrf }}
...
<div class="modal fade" tabindex="-1" role="dialog" id="modal-konfirmasi-dialog">
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{ site_url('sktm_sekolah/konfirmasi') }}" method="POST" id="form-konfirmasi">
					{{$csrf}}
					<div class="form-group">
